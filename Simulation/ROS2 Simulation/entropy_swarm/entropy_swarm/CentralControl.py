"""
Created by Jason Carnahan - 5/14/2021 for AirSim 

Edited by Tara Moore - 2025 for AirSim 

Edited by Juan José Molina - March 2026 for ROS2 
...
class CentralControl:
    ...
    def __init__(self, strategy):
        ...
        self.Vmax = 0.20   # Max Speed of the UAVs
        self.maxDistance = 2.70  # Max Survey Distance between UAVs
        self.minDistance = 0.40  # Min physical distance between UAVs
        self.waypoints = [[-1, 1], [1, 1], [1, -1], [-1, -1]] # ROS2 CFs
        #self.waypoints = [[25, 100], [75, 100], [75, 50], [25, 50]] # Airsim data
    ...
    def call_uav_update(self):
        ...
        for uav in self.uavs:
            ...
            print("THETA:", [None if u.theta is None else round(float(u.theta), 2) for u in self.uavs])
    ...
    def uav_init(self):
        ...
        """
        for uav in self.uavs:
            move = uav.strategy.move_by_heading(0, 0, 0.5, 1, 0)
    ...
    def entropyFormation(self):
        ...
        commandDuration = 0.3 # How long each movement action takes (in s?) changes from 1 to 0.1 for smoother movement
        ...
        z = 0.5  # Consistent height

        # MOVED THIS HERE Project 3
        waypoint_offset = 1 * self.minDistance  # The uav will seek the waypoint with this off set so they dont collide into the same point
        # In the future, may one to try and calculate the center of mass of the swarm and use that???
        closeEnough = 0.30   # offset from waypoint so the uav does not have to be exactly on it
        #print(f"[CC] entropyFormation: len(uavs)={len(self.uavs)} waypoints={self.waypoints}")
        ...
        with open(self._csv_path, "w", newline="") as csvfile:
            ...
            for i in range(1, len(self.uavs) + 1):
                header += [f"X{i}", f"Y{i}", f"EntropyCF{i}"]
