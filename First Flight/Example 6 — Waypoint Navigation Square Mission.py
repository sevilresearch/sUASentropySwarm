...
def _go_loop():
    global _go_running
    try:
        tol = _safe_float(tol_entry.get(), 0.10)        # arrival radius (m)
...
def _smooth_land(vel=0.20):
    """Stop any motion thread, then land gently."""
    global mc, _go_running, _go_thread
    # Stop the Go loop cleanly
    _go_running = False
    try:
        if _go_thread and _go_thread.is_alive():
            _go_thread.join(timeout=1.0)
    except Exception:
        pass

    # Gentle land (same API as your manual Land button, but slower)
    try:
        if mc is not None:
            mc.land(height=0.0, velocity=vel)  # default vel in cflib is ~0.3; we use 0.20
            mc = None
        _log_stop_and_save()  # keep your CSV behavior
        status_var.set("Status: Landed (auto)")
    except Exception as e:
        status_var.set(f"Status: Land error — {e}")



# ---- UI-thread helpers to keep Tkinter + CF calls safe ----
def _ui_set_target_and_go(tx, ty, tz):
    # This runs on the Tkinter main thread
    x_entry.delete(0, tk.END); x_entry.insert(0, f"{tx:.3f}")
    y_entry.delete(0, tk.END); y_entry.insert(0, f"{ty:.3f}")
    z_entry.delete(0, tk.END); z_entry.insert(0, f"{tz:.3f}")
    start_go()
...
# -------- Autonomous square mission (minimal addition) --------
_mission_thread = None
_mission_running = False

def _wait_until_arrived_or_timeout(tol, timeout_s):
    """Poll current error until within tol or timeout."""
    t0 = time.time()
    while time.time() - t0 < timeout_s and running:
        with _pos_lock:
            ex = target['x'] - pos['x']
            ey = target['y'] - pos['y']
            ez = target['z'] - pos['z']
...
def _mission_square_loop():
    global _mission_running
    try:
        L = _safe_float(square_len_entry.get(), 1.0)
        tol = _safe_float(tol_entry.get(), 0.08)
        leg_timeout = max(5.0, _safe_float(timeout_entry.get(), 25) / 2.0)

        # Ensure airborne (schedule on UI thread and wait for it to run)
        if mc is None:
            t_evt = threading.Event()
            root.after(0, _ui_takeoff_and_signal, t_evt)
            t_evt.wait(timeout=3.0)  # small wait so takeoff command is issued
            time.sleep(0.8)

        # Need a home reference
        if home['x'] is None:
            status_var.set('Status: No home position yet')
            return
        ox, oy = home['x'], home['y']
        tz = _safe_float(z_entry.get(), 0.5)
        ...
        for (tx, ty, tz) in waypoints:
            # 1) Set target & start_go on UI thread, wait until scheduled
            done = threading.Event()
            root.after(0, _ui_apply_target_and_start_go, tx, ty, tz, done)
            done.wait(timeout=2.0)

            # 2) Wait for motion to actually start (go thread sets _go_running)
            t0 = time.time()
            while (not _go_running) and (time.time() - t0 < 2.0):
                time.sleep(0.02)

            # 3) Wait for arrival OR timeout
            t_leg = time.time()
            reached = False
            while (time.time() - t_leg < leg_timeout) and running:
                with _pos_lock:
                    ex = target['x'] - pos['x']
                    ey = target['y'] - pos['y']
                    ez = target['z'] - pos['z']
            ...
            # 4) Give the go-thread up to 2s to finish and clear _go_running
            t_clear = time.time()
            while _go_running and (time.time() - t_clear < 2.0):
                time.sleep(0.02)

            if not reached:
                status_var.set('Status: Leg timeout — aborting mission')
                break

            time.sleep(0.5)  # tiny dwell

        # Smooth land (use your normal land path on UI thread)
        root.after(0, do_land)

    finally:
        _mission_running = False
        try:
            run_square_btn.config(state=tk.NORMAL)
        except Exception:
            pass
...
# ---- Run these ONLY on the Tk (UI) thread ----
def _ui_apply_target_and_start_go(tx, ty, tz, done_evt=None):
    # Set entries & start move on the main thread
    x_entry.delete(0, tk.END); x_entry.insert(0, f"{tx:.3f}")
    y_entry.delete(0, tk.END); y_entry.insert(0, f"{ty:.3f}")
    z_entry.delete(0, tk.END); z_entry.insert(0, f"{tz:.3f}")
    start_go()
...
def _ui_takeoff_and_signal(done_evt=None):
    do_takeoff()
    if done_evt is not None:
        try: done_evt.set()
        except Exception: pass
...
# --- Autonomous square controls (minimal) ---
ttk.Label(main, text="Square side (m):").grid(row=row, column=0, sticky="e", padx=4)
square_len_entry = ttk.Entry(main, width=8)
square_len_entry.insert(0, "1.00")
square_len_entry.grid(row=row, column=1, sticky="w")

def run_square():
    global _mission_thread, _mission_running
    if _mission_running:
        return
    _mission_running = True
    run_square_btn.config(state=tk.DISABLED)
    _mission_thread = threading.Thread(target=_mission_square_loop, daemon=True)
    _mission_thread.start()

run_square_btn = ttk.Button(main, text="Run Square (auto)", command=run_square)
run_square_btn.grid(row=row, column=2, padx=(8, 0), sticky="w")
row += 1
...
tol_entry = ttk.Entry(main, width=8); tol_entry.insert(0, "0.10"); tol_entry.grid(row=row, column=5, sticky="w")
